...
import { Building2, Store, Utensils, ShoppingBag } from 'lucide-react';
...
export const MSMEDashboard: React.FC<MSMEDashboardProps> = ({
  ...
  setBusinessProfile: propSetBusinessProfile
}) => {
  const [localBusiness, setLocalBusiness] = useState<BusinessProfile>(mockDemoBusiness);
  const businessProfile = propBusinessProfile || localBusiness;
  const setBusinessProfile = propSetBusinessProfile || setLocalBusiness;
  ...
  const runAnalysis = async () => {
    setLoading(true);
    try {
      const data = await apiService.analyzeMSME(businessProfile);
      setAnalysis(data);
    } catch (e) {
      console.error(e);
    } finally {
